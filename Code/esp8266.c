...
int32_t esp8266_connect_ap(char* ssid,char* pswd)
{
	...
	if(esp8266_find_str_in_rx_packet("WIFI CONNECTED",10000))	
...
int32_t esp8266_create_ap(char* name,char* pass,uint8_t chl,uint8_t ecn)
{

	char buf[64]={0};
	
    //设置为SoftAP+Station 模式	
	esp8266_send_at("AT+CWMODE=2\r\n"); 
	...
	//建立无线热点
	sprintf((char*)buf,"AT+CWSAP=\"%s\",\"%s\",%d,%d\r\n",name,pass,chl,ecn);
